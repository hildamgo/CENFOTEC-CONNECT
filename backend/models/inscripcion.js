...
// Nota: la Actividad todavía no vive en MongoDB (ese módulo no está migrado),
// así que el frontend nos manda los datos de la actividad ya resueltos.
...
async function buscarInscripciones(filtrosQuery) {
    const inscripciones = await coleccionInscripciones();
    const { buscar, estado, categoria, fecha } = filtrosQuery;
    const filtro = {};
